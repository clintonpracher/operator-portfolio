...
at_a_glance:
  ...
  - Built to run my AI-native business on one governed stack
...
When I started building my AI-native business, I did not want another pile of AI tools. I wanted one operating model I could trust to run revenue, delivery, and operating cadence as the business grew. So I built an **Operator Control Plane** on one spine: a versioned skills library, {{ site.data.stats.cursor_automations }} governed automations on an agentic control layer, registry-led identity in Notion, and an orchestration agent coordinating business and personal operations.
...
**Registered execution surfaces:** Orchestration, control layer, audit layer, and MCP integrations each hold an explicit role, not a popularity contest.

The swimlane diagram maps how the business actually runs: judgment up top, client surfaces and orchestration across the spine, business systems and registry in the middle, evidence and versioned procedures underneath, and MCP integrations routed in from the perimeter.
...
| **Running today** | My AI-native business, career pipeline, and operating cadence on explicit decision rights |
...
The registry could be Notion or Salesforce. Execution could be Cursor or an internal agent platform. Decision rights do not change: one writer per workflow, evidence on the record, audit separate from execution. I have made those calls at portfolio scale. This page shows them running on the business I operate today.
...
That is the difference between collecting AI tools and running a business where decision rights are explicit.
