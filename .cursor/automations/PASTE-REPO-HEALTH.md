...
Write Sweep Log row title Repo Health (no date in title). Set Repository property to slug. Clint Action per fleet-health-human-labels (All clear / FYI only / August queue / Needs you / Broken).
...
**Path note (`operator-portfolio`):** Federator and `--slug operator-portfolio` prefer `~/operator-portfolio` when that clone exists; `--from-cwd` in the monorepo folder `marketing-assets/operator-portfolio` may warn "Not a git repository" — expected unless you open the clone as the Cursor workspace.
